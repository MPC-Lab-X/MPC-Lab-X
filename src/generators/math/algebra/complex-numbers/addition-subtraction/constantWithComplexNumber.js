...
const generateProblem = (options) => {
  ...
  if (
    ...
    operation = Math.random() < 0.5 ? "addition" : "subtraction"; // Randomly choose addition or subtraction
  ...
  const problemText =
    operation === "addition"
      ? `${constant} + (${real} ${imaginary < 0 ? "-" : "+"} ${Math.abs(
          imaginary
        )}i)`
      : `${constant} - (${real} ${imaginary < 0 ? "-" : "+"} ${Math.abs(
          imaginary
  ...
  const steps = [
    ...
    {
      type: "formula",
      value: `${constant} ${operation === "addition" ? "+" : "-"} ${real} ${
        imaginary < 0 ? "-" : "+"
